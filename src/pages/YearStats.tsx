...
export default function YearStats() {
  ...
  const monthlyData = useMemo(() => {
    const allMonths = [
      ...new Set([...yearlyExpenses.map((e) => e.month), ...yearlyEarnings.map((e) => e.month)]),
        ...
        <>
          <div className="mb-6 bg-white rounded-lg shadow-sm p-4">
            ...
            <ChartContainer config={barConfig} className="h-[300px] w-full">
              <BarChart accessibilityLayer data={monthlyData} margin={{ top: 25, bottom: 10 }}>
                <CartesianGrid vertical={false} />
                <ChartTooltip
                  cursor={false}
                  content={
                    <ChartTooltipContent hideLabel hideIndicator formatter={(v) => `${v} €`} />
                  }
                />
                <Bar dataKey="balance">
                  <LabelList
                  ...
                  {monthlyData.map((item) => (
                    <Cell key={item.month} fill={item.balance >= 0 ? "#3f8600" : "#cf1322"} />
                  ))}
                </Bar>
              </BarChart>
            </ChartContainer>
