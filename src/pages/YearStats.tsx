...
import { setIsLoading, MONTHS } from '../services/utils';
...
export default function YearStats() {
	...
	const savingRate = useMemo(
		() => totalSumEarnings > 0 ? (100 * (totalSumEarnings + totalSumExpenses)) / totalSumEarnings : 0,
	...
	const pieExpenses = useMemo(() => buildCategoryData(yearlyExpenses), [yearlyExpenses, categories]);
	const pieEarnings = useMemo(() => buildCategoryData(yearlyEarnings), [yearlyEarnings, categories]);

	const includedCategoriesIds = useMemo(
		() => categories.filter(c => (c.type === 'earning' || c.type === 'expense') && !c.excludeFromBudget).map(c => c.id),
		[categories]
	);
	...
	const freezeYear = useCallback(async () => {
		setIsLoading(true);
		try {
			const yearTransactions: Transaction[] = await DataBaseClient.Transaction.get({ year });
			...
			months
				...
				.forEach(month => {
					...
					const getMonthSummary = (txns: Transaction[]) => {
						...
						return cats.map(cat => ({
							categoryId: cat,
							total: txns.filter(t => t.category === cat).reduce((acc, curr) => acc + curr.amount, 0),
						}));
					};
					yearStats.push({
						month, year, type: 'earning',
			...
			if (existingYearStats.length) {
				await DataBaseClient.Stats.bulkDelete(existingYearStats.map(s => s.id));
			}
		...
		<div className="p-4 max-w-2xl mx-auto">
			...
			<div className="grid grid-cols-4 gap-2 mb-6">
				<div className="flex flex-col items-center p-2 rounded-lg bg-white shadow-sm">
					...
					<span className="text-base font-bold" style={{ color: balance >= 0 ? '#3f8600' : '#cf1322' }}>
						{Math.round(balance)} €
					</span>
				</div>
				<div className="flex flex-col items-center p-2 rounded-lg bg-white shadow-sm">
					<span className="text-xs text-muted-foreground">Earnings</span>
					<span className="text-base font-bold text-earning">{Math.round(totalSumEarnings)} €</span>
				</div>
				<div className="flex flex-col items-center p-2 rounded-lg bg-white shadow-sm">
					<span className="text-xs text-muted-foreground">Expenses</span>
					<span className="text-base font-bold text-expense">{Math.round(-totalSumExpenses)} €</span>
				</div>
				<div className="flex flex-col items-center p-2 rounded-lg bg-white shadow-sm">
					<span className="text-xs text-muted-foreground">Saving</span>
					<span className="text-base font-bold" style={{ color: savingRate > 0 ? '#3f8600' : '#cf1322' }}>
			...
			{hasData ? (
				...
				<div className="text-center py-12">
					<p className="text-muted-foreground mb-2">No data available for this year</p>
					<p className="text-muted-foreground mb-4">Freeze the year to generate stats</p>
