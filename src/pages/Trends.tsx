	...
	const yearlyTotals = useMemo(() => {
		return years.map((year) => {
			...
			return {
				...
				savingsRate:
					earnings > 0 ? Math.round((balance / earnings) * 1000) / 10 : 0,
			};
		});
	}, [stats, years]);
...
function SavingsRateTrend({ data }: { data: YearlyTotal[] }) {
	...
	if (data.length === 0) {
		return (
			<EmptyCard
				title="Savings Rate"
				message="Freeze a year to see savings rate."
			/>
		);
	}
		...
		<div className="mb-6 bg-white rounded-lg shadow-sm p-4">
			<div className="flex items-start justify-between mb-1">
				<div>
					...
					<p className="text-xs text-muted-foreground">
						Share of earnings kept, by year
					</p>
				</div>
				<ChartTypeToggle value={chartType} onChange={setChartType} />
			</div>
					...
					>
						...
						<Bar dataKey="savingsRate" name="Savings %" radius={3}>
							{data.map((d) => (
								<Cell
									key={d.year}
									fill={d.savingsRate >= 0 ? EARNING : EXPENSE}
								/>
							))}
						</Bar>
					</BarChart>
	...
	const data = useMemo(() => {
		return MONTHS.map((month) => {
			...
			years.forEach((year) => {
				const earnings =
					stats.find(
						(s) => s.year === year && s.month === month && s.type === "earning",
					)?.total || 0;
				const expenses =
					stats.find(
						(s) => s.year === year && s.month === month && s.type === "expense",
					)?.total || 0;
				let v = 0;
		...
		<div className="mb-6 bg-white rounded-lg shadow-sm p-4">
			<div className="flex items-start justify-between mb-1">
				<div>
					...
					<p className="text-xs text-muted-foreground">
						Same months overlaid across years
					</p>
				</div>
				<ChartTypeToggle value={chartType} onChange={setChartType} />
			</div>
