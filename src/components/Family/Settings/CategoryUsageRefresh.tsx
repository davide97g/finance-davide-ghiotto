...
export default function CategoryUsageRefresh() {
	...
	const handleRefresh = async () => {
		...
		try {
			...
			const allTransactions = await Promise.all(
				windows.map(w => DataBaseClient.Transaction.get({ type: 'expense', month: w.month, year: w.year }))
			);
			...
			openNotificationWithIcon('success', 'Success', 'Category usage refreshed');
		} catch (err) {
			console.error(err);
			openNotificationWithIcon('error', 'Error', 'Failed to refresh category usage');
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<div className="text-left">
			...
			<p className="text-xs text-muted-foreground mt-2">{formatLastRefreshed()}</p>
		</div>
	);
}
