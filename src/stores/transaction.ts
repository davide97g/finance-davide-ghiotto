...
const compareTransactions = (t1: Transaction, t2: Transaction, ascending: boolean): number => {
  const d1 = dayMs(t1);
  const d2 = dayMs(t2);
  if (d1 !== d2) {
...
export const useTransactionStore = create<TransactionState>((set, get) => ({
  ...
  updateTransaction: (transaction: Transaction) => {
    set((state) => {
      ...
      if (transaction.type === "expense") {
        const next = state.expenses.map((t) => (t.id === transaction.id ? transaction : t));
        return { expenses: sortByDate(next, ascending) };
      }
      const next = state.earnings.map((t) => (t.id === transaction.id ? transaction : t));
  ...
  bulkUpdateTransactions: (transactions: Transaction[]) => {
    set((state) => {
      ...
      transactions.forEach((transaction) => {
        if (transaction.type === "expense") {
          newExpenses = newExpenses.map((t) => (t.id === transaction.id ? transaction : t));
        } else {
          newEarnings = newEarnings.map((t) => (t.id === transaction.id ? transaction : t));
        }
      });
      const ascending = state.sorting === "ascending";
      return {
